...
fn test_template_content_quality() {
    ...
    for lang_name in &["rust", "python", "javascript"] {
        ...
        // Common placeholders
        assert!(template.contains("{{title}}"), "Template for {} missing {{title}}", lang_name);
        assert!(template.contains("{{id}}"), "Template for {} missing {{id}}", lang_name);
        assert!(template.contains("{{description}}"), "Template for {} missing {{description}}", lang_name);
        assert!(template.contains("{{#each scenarios}}"), "Template for {} missing scenario iteration", lang_name);
        
        // Check for user implementation markers
        assert!(template.contains("START USER IMPLEMENTATION"), "Template for {} missing start marker", lang_name);
        assert!(template.contains("END USER IMPLEMENTATION"), "Template for {} missing end marker", lang_name);
    }
}
