...
fn test_config_init_project_creates_structure() {
    ...
    let config = Config::init_project_in_dir(temp_dir.path().to_str().unwrap()).expect("Failed to initialize project");
...
fn test_config_load_existing() {
    ...
    // Load config
    let loaded_config = Config::load_from_dir(temp_dir.path().to_str().unwrap()).expect("Failed to load config");
...
fn test_config_save_modifications() {
    ...
    // Save changes
    config.save_in_dir(temp_dir.path().to_str().unwrap()).expect("Failed to save config");
...
fn test_scenario_file_updates() {
    ...
    let (_temp_dir, mut manager) = create_test_manager_with_config(config);
        ...
        assert!(test_content.contains("AUTO-GENERATED TEST"), "Should contain test generation marker");
        assert!(test_content.contains(&use_case_id), "Should contain use case ID");
        assert!(test_content.contains("Test Scenario"), "Should contain scenario name");
}
