...
fn test_cli_auto_init_with_language() -> anyhow::Result<()> {
    ...
    let python_template = temp_dir.path().join(".config/.mucm/templates/lang-python/test.hbs");
...
fn test_cli_with_custom_config_directories() -> anyhow::Result<()> {
    ...
    let custom_use_case_file = temp_dir.path().join("custom/use-cases/custom/UC-CUS-001.md");
    assert!(custom_use_case_file.exists(), "Use case should be in custom directory");
