...
fn test_template_preservation_features() {
    with_temp_dir(|_temp_dir| {
        ...
        assert!(unsupported_result.unwrap_err().to_string().contains("Unsupported language"));
    });
}
...
fn test_marker_granularity() {
    with_temp_dir(|_temp_dir| {
        ...
        println!("Rust content START markers: {}, END markers: {}, test functions: {}", 
                start_markers, end_markers, test_functions);
        
        assert_eq!(start_markers, 2, "Should have START marker for each scenario");
        // There might be an extra END marker in the template structure, so let's check it's at least 2
        assert!(end_markers >= 2, "Should have at least END marker for each scenario");
        assert_eq!(test_functions, 2, "Should have test function for each scenario");
        assert_eq!(start_markers, test_functions, "Each test should have its own markers");
        ...
        println!("Python content START markers: {}, END markers: {}, test methods: {}", 
                py_start_markers, py_end_markers, py_test_methods);
        
        assert_eq!(py_start_markers, 2, "Should have START marker for each scenario");
        assert!(py_end_markers >= 2, "Should have at least END marker for each scenario");
        assert_eq!(py_test_methods, 2, "Should have test method for each scenario");
        assert_eq!(py_start_markers, py_test_methods, "Each test should have its own markers");
    });
}