...
fn test_conditional_template_loading_python() {
    ...
    let mut data = HashMap::new();
    data.insert("title".to_string(), serde_json::json!("Test Use Case"));
    data.insert("title_snake_case".to_string(), serde_json::json!("test_use_case"));
    data.insert("id".to_string(), serde_json::json!("UC-TST-001"));
...
fn test_no_tests_generated_without_scenarios() {
    with_temp_dir(|_temp_dir| {
        ...
        let _use_case_id = manager.create_use_case(
        ...
        assert!(!test_file.exists(), "Test file should not exist for use case without scenarios");
    });
}
...
fn test_tests_generated_with_scenarios_rust() {
    with_temp_dir(|_temp_dir| {
        ...
        let _scenario_id = manager.add_scenario_to_use_case(
            use_case_id.clone(),
        ...
        assert!(test_file.exists(), "Test file should exist after adding scenario");
...
fn test_tests_generated_with_scenarios_python() {
    with_temp_dir(|_temp_dir| {
        ...
        assert!(test_file.exists(), "Python test file should exist after adding scenario");
...
fn test_no_tests_when_disabled() {
    with_temp_dir(|_temp_dir| {
        ...
        assert!(!test_file.exists(), "Test file should not exist when test generation is disabled");
    });
}
...
fn test_one_test_per_scenario() {
    with_temp_dir(|_temp_dir| {
        ...
        let _scenario_1 = manager.add_scenario_to_use_case(
            use_case_id.clone(),
            "First Scenario".to_string(),
            Some("First test scenario".to_string()),
        ).unwrap();
        
        let _scenario_2 = manager.add_scenario_to_use_case(
            use_case_id.clone(),
            "Second Scenario".to_string(),
            Some("Second test scenario".to_string()),
        ).unwrap();
        
        let _scenario_3 = manager.add_scenario_to_use_case(
            use_case_id.clone(),
        ...
        assert!(content.contains("fn test_uc_tes_001_s01()"), "Should have test for first scenario");
        
        // If more than one scenario, check for additional functions
        if test_count > 1 {
            assert!(content.contains("fn test_uc_tes_001_s02()"), "Should have test for second scenario"); 
        }
        if test_count > 2 {
            assert!(content.contains("fn test_uc_tes_001_s03()"), "Should have test for third scenario");
        }
        ...
        assert!(content.contains("START USER IMPLEMENTATION"), "Should contain new markers");
        assert!(content.contains("END USER IMPLEMENTATION"), "Should contain new markers");
    });
}