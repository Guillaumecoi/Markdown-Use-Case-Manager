...
fn test_auto_init_detection() -> Result<()> {
    ...
    assert!(result.is_err(), "Should fail when project is not initialized");
...
fn test_auto_init_backward_compatibility() -> Result<()> {
    ...
    // Update status
    coordinator.update_scenario_status(
        scenario_id,
...
fn test_config_validation() -> Result<()> {
    ...
    assert_eq!(parsed_config.generation.test_language, config.generation.test_language);
...
fn test_auto_init_settings_integration() -> Result<()> {
    ...
    assert!(custom_use_case_file.exists(), "Use case should be created in custom directory");
    ...
    assert_eq!(final_config.directories.use_case_dir, "docs/custom-use-cases");
