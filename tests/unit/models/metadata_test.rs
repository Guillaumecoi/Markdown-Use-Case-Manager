...
fn test_metadata_default() {
    ...
    let time_diff = metadata2
        .created_at
        .signed_duration_since(metadata1.created_at)
        .num_milliseconds();
    assert!(
        time_diff.abs() < 100,
        "Default and new should create similar timestamps"
    );
}
...
fn test_metadata_timestamp_consistency() {
    ...
    for _ in 0..3 {
        ...
        metadata.touch();
        assert_eq!(
            metadata.created_at, original_created,
    ...
    metadata.touch();
    assert!(
        metadata.updated_at > first_update,
        "updated_at should keep updating"
    );
}
