...
pub fn run() -> Result<()> {
    ...
    if cli.interactive || matches!(cli.command, Some(Commands::Interactive)) || cli.command.is_none() {
    ...
    match cli.command.unwrap() {
        Commands::Init { language } => {
            handle_init_command(&mut runner, language)
        }
        ...
        Commands::List => {
            handle_list_command(&mut runner)
        }
        Commands::Languages => {
            handle_languages_command()
        }
        Commands::Status => {
            handle_status_command(&mut runner)
        }
