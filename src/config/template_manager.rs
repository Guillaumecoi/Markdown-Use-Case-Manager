...
impl TemplateManager {
    ...
    pub fn copy_templates_to_config(base_dir: &str) -> Result<()> {
        ...
        let config_templates_dir = base_path.join(Config::CONFIG_DIR).join(Config::TEMPLATES_DIR);
        ...
        // Copy methodologies
        Self::copy_methodologies(&source_templates_dir, &config, &config_templates_dir, &config_methodologies_dir)?;
    ...
    fn copy_methodologies(
        ...
        if !source_methodologies.exists() {
            anyhow::bail!("Source methodologies directory not found at {:?}", source_methodologies);
        }
